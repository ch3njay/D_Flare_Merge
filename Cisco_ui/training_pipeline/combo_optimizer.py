...
def _as_cpu_estimator(est):
    ...
    if "xgb" in name or "xgboost" in name:
        ...
        _set_if_has(e, tree_method="hist", n_jobs=1, verbosity=0)
...
class ComboOptimizer:
    ...
    def _eval_and_compose(self, model, kind: str, estimators: List[Tuple[str, Any]]):
        ...
        # 評估個別子模型成績
        submodel_metrics = self._evaluate_submodels(model, estimators)
    ...
    def _evaluate_submodels(self, ensemble_model, estimators: List[Tuple[str, Any]]) -> Dict[str, Dict]:
        """評估集成模型中每個子模型的個別成績。"""
        submodel_results = {}
        ...
        fitted_estimators = ensemble_model.estimators_
        estimator_names = [name for name, _ in estimators]
        
        for name, fitted_est in zip(estimator_names, fitted_estimators):
            submodel_results[name] = self._evaluate_single_model(fitted_est, name)
                
        return submodel_results
    
    def _evaluate_single_model(self, model, name: str) -> Dict[str, Any]:
        """評估單一模型的成績。"""
        try:
            y_pred = model.predict(self.X_valid)
            proba = None
            ...
            return _compute_metrics(self.task_type, self.y_valid, y_pred, proba)
            
        except Exception as e:
            print(f"⚠️ 評估子模型 {name} 時失敗：{e}")
            return {"error": str(e)}
    ...
    def _write_final_txt(self, kind: str, metrics: Dict[str, Any], composition: Dict[str, Any]) -> None:
        ...
        # 集成整體成績
        lines.append(f"  整體成績：ACC={metrics['acc']:.6f} | F1={metrics['f1']:.6f} | AUC={auc_str}")
        
        # 子模型個別成績
        self._add_submodel_results(lines, metrics)
        
        # 集成混淆矩陣
        self._add_ensemble_confusion_matrix(lines, metrics)
    ...
    def _add_submodel_results(self, lines: List[str], metrics: Dict[str, Any]) -> None:
        """添加子模型成績到報告中。"""
        submodel_metrics = metrics.get("submodel_metrics", {})
        if not submodel_metrics:
            return
            
        lines.append("\n📊 子模型個別成績：")
        for name, sub_metrics in submodel_metrics.items():
            if "error" in sub_metrics:
                lines.append(f"  ❌ {name}: 評估失敗 - {sub_metrics['error']}")
            else:
                self._add_single_submodel_result(lines, name, sub_metrics)
    
    def _add_single_submodel_result(self, lines: List[str], name: str, sub_metrics: Dict[str, Any]) -> None:
        """添加單個子模型的成績到報告中。"""
        sub_auc = sub_metrics.get("auc", np.nan)
        sub_auc_str = "nan" if np.isnan(sub_auc) else f"{sub_auc:.6f}"
        
        lines.append(f"  📈 {name}:")
        acc = sub_metrics['acc']
        f1 = sub_metrics['f1']
        lines.append(f"      ACC={acc:.6f} | F1={f1:.6f} | AUC={sub_auc_str}")
        
        # 混淆矩陣
        cm = sub_metrics.get("confusion_matrix")
        if cm is not None:
            lines.append("      混淆矩陣：")
            cm_lines = str(cm).split('\n')
            for cm_line in cm_lines:
                lines.append(f"        {cm_line}")
    
    def _add_ensemble_confusion_matrix(self, lines: List[str], metrics: Dict[str, Any]) -> None:
        """添加集成混淆矩陣到報告中。"""
        ensemble_cm = metrics.get("confusion_matrix")
        if ensemble_cm is not None:
            lines.append("\n📝 集成混淆矩陣：")
            cm_lines = str(ensemble_cm).split('\n')
            for cm_line in cm_lines:
                lines.append(f"  {cm_line}")
